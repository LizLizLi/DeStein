...
from modeling.modeling_mpt import MptPreTrainedModel, MptForCausalLM
import re

MAX_LENGTH = int(10000)  # Hardcoded max length to avoid infinite loop


class MPTActGeneration:

    def __init__(
        self,
        model: Union[str, Path, MptPreTrainedModel] = "mpt",
        tokenizer: str = "",
        seed: int = 42,
        alpha: float = 0.3,
        count: int = 20,
        ...
        parent_path = os.path.abspath(".") + "/data/act/mpt-7b/{count}/".format(
            count=count
        )
    ...
    def __repr__(self):
        return f'<MPTGenerator model_name_or_path="{self.model}">'
