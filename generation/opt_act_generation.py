...
from modeling.modeling_opt import OPTPreTrainedModel, OPTForCausalLM
import re
...
class OPTActGeneration:

    def __init__(
        self,
        model: Union[str, Path, OPTPreTrainedModel] = "opt",
        tokenizer: str = "",
        seed: int = 42,
        alpha: float = 0.3,
        count: int = 20,
        ...
        # load activations
        parent_path = os.path.abspath(".") + "/data/act/opt-6.7b/{count}/".format(
            count=count
        )
    ...
    def __repr__(self):
        return f'<OPTGenerator model_name_or_path="{self.model}">'
