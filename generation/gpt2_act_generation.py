...
from modeling.modeling_gpt2 import GPT2LMHeadModelWithAct


MAX_LENGTH = int(10000)  # Hardcoded max length to avoid infinite loop


class GPT2ActGeneration:
    ...
    def __init__(
        ...
        utils.set_seed(seed=16, n_gpu=n_gpu)

        # Set up model
        model_name_or_path = str(model)
        if isinstance(model, Path) or isinstance(model, str):
            model = GPT2LMHeadModelWithAct.from_pretrained(model_name_or_path)
        ...
        # load activations
        parent_path = os.path.abspath(".") + "/data/act/gpt2/{count}/".format(
            count=count
        )
        print(parent_path)
        ...
        if (
            ...
            all_train_layer_y = torch.cat(all_train_layer_y, dim=1)
            ...
            # all_head_wise_directions = torch.mul(
            #     all_head_wise_directions, matrix.squeeze(1)
            # )
    ...
    def __repr__(self):
        return f'<GPT2ActGenerator model_name_or_path="{self.model}">'
